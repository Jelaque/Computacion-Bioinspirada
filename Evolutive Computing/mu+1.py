...
def mu_lam(low,high,n,mu,lam,p,f,it,fit):
    ...
    while(ii < it):
        ...
        for i in range(0,lam):
            ...
            print(child,sigma_,'\n')
            values = np.vstack([values,child])
            sigma = np.vstack([sigma,sigma_])
            ap = np.hstack([ap,f(child)])

        print("Aptitude of the all population")
        print_ap(values,sigma,ap,mu+lam)

        M = np.hstack([values,sigma,ap.reshape(1,mu+lam).T])
        M = M[M[:,2*n].argsort(axis=0)]
        M = np.delete(M,slice(mu,mu+lam),axis=0)
...
mu_lam(-10,10,2,8,6,5,f,300,min)
